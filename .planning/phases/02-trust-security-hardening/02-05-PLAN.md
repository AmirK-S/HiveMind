...
autonomous: true
requirements: [TRUST-04, ACL-05, SEC-01, SEC-02, SEC-03]

must_haves:
  truths:
    - "add_knowledge rejects content classified as prompt injection before PII stripping"
    - "add_knowledge enforces anti-sybil burst detection via check_burst() after injection scan"
  ...
  key_links:
    ...
    - from: "hivemind/server/tools/add_knowledge.py"
      to: "hivemind/security/rate_limit.py"
      via: "check_burst() for anti-sybil enforcement"
      pattern: "check_burst"
    - from: "hivemind/server/tools/add_knowledge.py"
...
# Uses Redis ZSET window from rate_limit.py.
...
<verification>
1. add_knowledge.py imports InjectionScanner, check_burst, get_redis_connection, and AutoApproveRule
2. add_knowledge flow: validation -> auth -> injection scan -> burst check -> PII strip -> hash -> auto-approve check -> insert (pending or direct)
...
6. Grep add_knowledge.py for "check_burst" confirms rate limiting enforcement is present
</verification>

<success_criteria>
- Prompt injection is blocked before PII stripping in add_knowledge (SEC-01 wired)
- Anti-sybil burst detection rejects excessive contributions via check_burst (SEC-03 enforced per-request)
