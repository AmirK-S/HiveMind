...
must_haves:
  ...
  artifacts:
    ...
    - path: "hivemind/security/rate_limit.py"
      ...
      exports: ["init_rate_limiter", "TIER_LIMITS", "check_burst", "get_redis_connection"]
...
- Store the redis connection in a module-level `_redis_conn` variable for anti-sybil use

`def get_redis_connection()`:
- Returns the module-level `_redis_conn` (or `None` if `init_rate_limiter` hasn't been called yet)
- This allows other modules (e.g. add_knowledge) to access the Redis connection for burst checking
...
Include docstrings referencing INFRA-04 and SEC-03. Note that the raw API key is NEVER stored in the database (Pitfall: never store raw keys — research anti-pattern).
  ...
  <verify>
Run `python -c "from hivemind.security.rate_limit import TIER_LIMITS, init_rate_limiter, check_burst, get_redis_connection; assert get_redis_connection() is None; print(f'Tiers: {list(TIER_LIMITS.keys())}')"` — should print tiers (get_redis_connection returns None before init).
