...
must_haves:
  ...
  artifacts:
    ...
    - path: "hivemind/server/auth.py"
      provides: "Dual auth: JWT + hm_-prefixed API key support"
      contains: "decode_token_async"
  ...
  key_links:
    ...
    - from: "hivemind/server/auth.py"
      to: "hivemind/security/api_key.py"
      via: "decode_token_async detects hm_ prefix and calls validate_api_key"
      pattern: "hm_.*validate_api_key"
---
...
<task type="auto">
  <name>Task 3: Wire API key authentication into auth.py alongside JWT</name>
  <files>hivemind/server/auth.py</files>
  <action>
Modify `hivemind/server/auth.py` to support both JWT tokens and `hm_`-prefixed API keys in the same `decode_token()` entry point.

**Add optional `tier` field to `AuthContext`:**
```python
@dataclass
class AuthContext:
    org_id: str
    agent_id: str
    tier: str | None = None  # Set when authenticated via API key (INFRA-04)
```

**Modify `decode_token()` to detect API keys:**

Before the existing JWT decode logic, add an API key detection branch:
...
def decode_token(token: str) -> AuthContext:
    ...
    # Existing JWT decode logic below (unchanged)
    ...
```

**Add `_decode_api_key()` private helper:**

```python
import asyncio

def _decode_api_key(raw_key: str) -> AuthContext:
    """Validate an hm_-prefixed API key and return AuthContext.

    Uses asyncio.run() or get_event_loop().run_until_complete() to call
    the async validate_api_key from a sync context. If already in an async
    context (e.g. called from an MCP tool), use the existing event loop.

    Raises:
        ValueError: If the API key is invalid, inactive, or not found.
    """
    from hivemind.security.api_key import validate_api_key, increment_request_count

    # Run async validation — handle both sync and async calling contexts
    try:
    ...
    if loop and loop.is_running():
        # We're inside an async context (e.g. MCP tool handler)
        # Create a task and use a future — but since decode_token is sync,
        # use a new thread to avoid blocking the event loop
        import concurrent.futures
        with concurrent.futures.ThreadPoolExecutor() as pool:
            result = loop.run_in_executor(pool, lambda: asyncio.run(validate_api_key(raw_key)))
            # Since we can't await here, use the synchronous fallback
            result = asyncio.run(validate_api_key(raw_key))
    else:
        result = asyncio.run(validate_api_key(raw_key))
    ...
    # Increment request count (fire and forget — don't block auth)
    try:
        if loop and loop.is_running():
            asyncio.run(increment_request_count(result["api_key_id"]))
        else:
            asyncio.run(increment_request_count(result["api_key_id"]))
    except Exception:
        pass  # Best-effort counter increment
...
NOTE: The async-from-sync bridging is necessary because `decode_token()` is called synchronously from tool handlers, but `validate_api_key()` is async (it queries the DB). The simplest correct approach: since MCP tool handlers are already async and extract the token before calling `decode_token()`, consider making `decode_token()` accept an optional pre-validated result. However, to keep the change minimal and avoid modifying all callers, use `asyncio.run()` which creates a new event loop in a new thread. If this proves problematic, a follow-up can refactor to `async def decode_token_async()`.

A simpler and more reliable approach: make `_decode_api_key` async and add `async def decode_token_async(token: str) -> AuthContext` as a parallel entry point. Keep `decode_token()` for JWT-only backward compatibility. Update the module docstring usage example to recommend `decode_token_async()` for tools that handle both JWT and API keys.

**Recommended simpler implementation:**

Add `async def decode_token_async(token: str) -> AuthContext`:
```python
async def decode_token_async(token: str) -> AuthContext:
    """Decode a JWT or validate an hm_-prefixed API key. Async version.

    Prefer this over decode_token() in async contexts (MCP tool handlers).
    """
    if token.startswith("hm_"):
        from hivemind.security.api_key import validate_api_key, increment_request_count
        ...
        if result is None:
            raise ValueError("Invalid or inactive API key")
        # Increment request count (best-effort)
        try:
    ...
    # Fall through to existing JWT logic
    return decode_token(token)
```

Update module docstring to show `decode_token_async()` as the preferred entry point for MCP tools.
  </action>
  <verify>
Run `python -c "from hivemind.server.auth import AuthContext, decode_token, decode_token_async; ctx = AuthContext(org_id='test', agent_id='a', tier='free'); assert ctx.tier == 'free'; print('API key auth wiring importable')"` — should succeed.
Grep `hivemind/server/auth.py` for `hm_` to confirm API key detection is present.
  </verify>
  <done>auth.py detects hm_-prefixed tokens and routes them through validate_api_key() to construct AuthContext with tier info. JWT tokens continue through the existing decode path unchanged. decode_token_async() provides the async entry point for MCP tool handlers (INFRA-04).</done>
</task>
...
6. `python -c "from hivemind.server.auth import decode_token_async; print('hm_ API key auth wiring present')"` — confirms decode_token_async is importable and handles hm_-prefixed keys alongside JWT
7. All files import without errors
</verification>
