...
must_haves:
  truths:
    ...
    - "A VHS .tape file exists that produces a demo GIF showing the contribute-search-feedback loop via curl REST calls (REST/MCP parity means this demonstrates the same workflow agents use via MCP)"
...
async def server_card():
    return JSONResponse({
        "serverInfo": {
            "name": "hivemind",
            "description": "Shared memory system for AI agents — contribute and retrieve knowledge from the collective commons",
            "version": "0.1.0"
        },
        "authentication": {
            "type": "apiKey",
...
Register this router in `hivemind/server/main.py` — add `app.include_router(well_known_router)` BEFORE the MCP mount. Note: this goes on the FastAPI `app` directly (not `api_router`), because the path starts with `/.well-known/` which is a top-level path.

**`glama.json`** — Place in repo root:
```json
{
...
The maintainers field should use the actual GitHub username (placeholder for now — executor should update with real username).
  </action>
  <verify>
...
<task type="auto">
  ...
  <action>
**`scripts/demo.tape`** — VHS script for reproducible demo GIF:
...
Note: The actual demo tape should be refined during execution to use real API responses. The tape file is a template — the executor should adjust curl commands to match the actual API and use environment variables for keys (Pitfall 7 from research: no hardcoded paths/keys).
...
2. **PulseMCP:** Visit pulsemcp.com/submit — fill in name (HiveMind), description, GitHub URL
3. **Glama.ai:** glama.json is in repo root — push to main, then claim ownership at glama.ai/mcp/servers
4. **mcp.so:** Open GitHub issue on modelcontextprotocol/servers — title: "Add HiveMind"
...
2. `test -f glama.json` — exists in repo root
3. `test -f scripts/demo.tape` — exists with VHS Output directive
